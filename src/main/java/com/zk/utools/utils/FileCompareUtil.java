...
public class FileCompareUtil {
    ...
    public static void main(String[] args) {
        int splitSize = 100;
        File[] srcFile = filtrateFile("data/njty_eids.txt", splitSize);
        File[] targetFile = filtrateFile("data/target.file", splitSize);
        compareFile(srcFile, targetFile, "data//out/result.txt", splitSize);
        ...
        try {
            ...
            for (int k = 0; k < spliteSize; k++) {
                if (targetFile[k].exists()) {
                    targetReaders[k] = new FileReader(targetFile[spliteSize]);
